// routes/products.js
// GET  /api/products          — list all products (with optional filter/sort query params)
// GET  /api/products/:id      — get single product
// POST /api/products          — add product (admin only — add admin check in production)
...
// ── GET ALL PRODUCTS ──────────────────────────────────────────────
// Query params: ?category=budget&sort=price-low&type=Portable
router.get('/', async (req, res) => {
  ...
  try {
    ...
    // Category price filter (done in memory since Firestore needs composite indexes for range queries)
    if (category === 'budget') products = products.filter(p => p.price < 3000);
    else if (category === 'mid') products = products.filter(p => p.price >= 3000 && p.price <= 7000);
    else if (category === 'premium') products = products.filter(p => p.price > 7000);
...
// ── SEED PRODUCTS (run once to populate Firestore) ────────────────
// POST /api/products/seed  — call this once from Postman/curl to populate DB
router.post('/seed', async (req, res) => {
  ...
  try {
    ...
    await batch.commit();
    res.json({ message: `✅ ${products.length} products seeded to Firestore.` });
  } catch (err) {
